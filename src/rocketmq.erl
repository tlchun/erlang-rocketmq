...
ensure_supervised_client(ClientId, Opts) ->
  rocketmq_client_sup:ensure_present(ClientId,
    [{"127.0.0.1", 9876}],
    Opts).
...
ensure_supervised_producers(ClientId, ProducerGroup,
    Topic, Opts) ->
  rocketmq_producers:start_supervised(ClientId,
    ProducerGroup,
    Topic,
    Opts).
...
send(Producers, Message) ->
  {_Partition, ProducerPid} =
    rocketmq_producers:pick_producer(Producers),
  rocketmq_producer:send(ProducerPid, Message).

send_sync(Producers, Message, Timeout) ->
  {_Partition, ProducerPid} =
    rocketmq_producers:pick_producer(Producers),
  rocketmq_producer:send_sync(ProducerPid,
    Message,
    Timeout).
