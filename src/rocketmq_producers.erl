...
start_link(ClientId, ProducerGroup, Topic,
    ProducerOpts) ->
...
init([ClientId, ProducerGroup, Topic, ProducerOpts]) ->
  ...
  RefTopicRouteInterval =
    maps:get(ref_topic_route_interval, ProducerOpts, 5000),
  erlang:send_after(RefTopicRouteInterval,
    self(),
    ref_topic_route),
  {ok,
...
handle_info({start_producer, BrokerAddrs, QueueSeq},
    State = #state{producers = Producers}) ->
  NewProducers = do_start_producer(BrokerAddrs,
    QueueSeq,
    Producers,
    State),
  ...
  case rocketmq_client_sup:find_client(ClientId) of
    {ok, Pid} ->
      erlang:send_after(RefTopicRouteInterval,
        self(),
        ref_topic_route),
      case rocketmq_client:get_routeinfo_by_topic(Pid, Topic)
      of
        {_, undefined} -> {noreply, State};
        {_, Payload} ->
          BrokerDatas1 = lists:sort(maps:get(<<"brokerDatas">>,
            Payload,
            [])),
          case BrokerDatas1 -- lists:sort(BrokerDatas) of
            [] -> {noreply, State};
            BrokerDatas2 ->
              QueueDatas = maps:get(<<"queueDatas">>,
                Payload,
                []),
              {NewQueueNums, NewProducers} =
                start_producer(QueueNums,
                  BrokerDatas2,
                  QueueDatas,
                  Producers,
                  State),
              ...
              {noreply,
                State#state{queue_nums = NewQueueNums,
...
maybe_start_producer(Pid, {Header, undefined},
    State = #state{topic = Topic}) ->
  log_error("Start topic:~p producer fail:~p",
    [Topic, maps:get(<<"remark">>, Header, undefined)]),
  Result = rocketmq_client:get_routeinfo_by_topic(Pid,
    <<"TBW102">>),
  maybe_start_producer(Pid, Result, State);
maybe_start_producer(_, {_, Payload},
    State = #state{producers = Producers}) ->
  BrokerDatas = maps:get(<<"brokerDatas">>, Payload, []),
  QueueDatas = maps:get(<<"queueDatas">>, Payload, []),
  {QueueNums, NewProducers} = start_producer(0,
    BrokerDatas,
    QueueDatas,
    Producers,
    State),
...
start_producer(Start, BrokerDatas, QueueDatas,
    Producers, State = #state{topic = Topic}) ->
  lists:foldl(fun (BrokerData,
    ...
    case maps:get(<<"perm">>, QueueData) =:= 4 of
      true ->
        log_error("Start producer fail tioic:~p permission "
        "denied:~p",
          [Topic]),
        {QueueNumAcc, ProducersAcc};
      false ->
        QueueNum = maps:get(<<"writeQueueNums">>,
          QueueData),
        QueueNumAcc1 = QueueNumAcc + QueueNum,
        ProducersAcc1 = lists:foldl(fun (QueueSeq,
            Acc) ->
          do_start_producer(BrokerAddrs,
            QueueSeq,
            Acc,
            State)
                                    end,
          ProducersAcc,
          lists:seq(QueueNumAcc,
            QueueNumAcc1
              - 1)),
...
do_start_producer(BrokerAddrs, QueueSeq, Producers,
    #state{workers = Workers, topic = Topic,
      producer_group = ProducerGroup,
      producer_opts = ProducerOpts}) ->
  Server = maps:get(<<"0">>, BrokerAddrs),
  {ok, Producer} = rocketmq_producer:start_link(QueueSeq,
    Topic,
    Server,
    ProducerGroup,
    ProducerOpts),
