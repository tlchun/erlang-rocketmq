...
handle_call({get_routeinfo_by_topic, Topic}, From,
    State = #state{opaque_id = OpaqueId, sock = Sock,
  ...
  case get_sock(Servers, Sock) of
    ...
    Sock1 ->
      Package =
        rocketmq_protocol_frame:get_routeinfo_by_topic(OpaqueId,
          Topic),
      gen_tcp:send(Sock1, Package),
      {noreply,
        next_opaque_id(State#state{requests =
        maps:put(OpaqueId, From, Reqs),
          sock = Sock1})}
  end;
...
handle_info(_Info, State) ->
  log_error("RocketMQ client Receive unknown message:~p~n",
    [_Info]),
...
handle_response(<<>>, State) ->
  {noreply, State, hibernate};
handle_response(Bin,
    State = #state{requests = Reqs, last_bin = LastBin}) ->
...
try_connect([{Host, Port} | Servers]) ->
  case gen_tcp:connect(Host, Port,
    [binary, {packet, raw}, {reuseaddr, true}, {nodelay, true}, {active, true}, {reuseaddr, true}, {send_timeout, 60000}], 60000) of
    {ok, Sock} -> %% 连接成功
      %% 设置发送接收的buffer
      tune_buffer(Sock),
      %% 转交进程控制全
